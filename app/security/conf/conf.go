...
func initConfRegister() {
	...
	if err != nil {
		klog.Fatalf("consul client init failed: %v", err)
	}

	client.RegisterConfigCallback("security/test.yaml", consul.AllocateUniqueID(), func(s string, cp consul.ConfigParser) {
		// map to config
		err = yaml.Unmarshal([]byte(s), &conf)
		if err != nil {
			panic(err)
		}
		klog.Info("config updated: ", s)
	})
}
